...
기본 `mapHalfSize=2500`이면 배열은 5000×5000, 25,000,000개 셀입니다. 원소 저장 공간은 약 23.8 MiB이며 컨테이너 여유 공간을 제외한 계산값입니다. 한 셀 조회는 직접 인덱싱하지만 영역 검사는 그 영역의 셀 수만큼 순회합니다. `ClearObjectTypeFromMap`의 비용은 전체 셀 수에 비례합니다.
...
| 렌더링 연결 | [MapInstancedMeshActor](../Source/Source/ShootingStar/Private/MapInstancedMeshActor.cpp)는 생성 위치를 메시 인스턴스로 연결하는 팀 공동 코드 |
...
후속 수정에서는 장애물, 주변 장애물, 장식의 외부 생성 반복문에 목표 수의 3배, 펜스에는 2배의 시도 예산을 적용하고 대체 위치 탐색 실패도 한 회차로 계산하도록 했습니다. 장식은 군집, 펜스는 패턴 단위로 성공 횟수와 부분 생성 로그를 기록합니다. 각 단위에서 생성되는 실제 인스턴스 수는 별도로 결정됩니다.
...
`ClearSpawnedResources`는 이미 파괴된 Actor를 유효성 검사로 건너뛰고 목록을 비웁니다. 채굴에 따른 크기 변경과 제거는 [ResourceActor.UpdateMesh_AfterHarvest](../Source/Source/ShootingStar/Private/ResourceActor.cpp)에서 처리합니다. 채굴 때마다 개별 점유 셀을 해제하는 방식은 아니며 자원 비트는 라운드 재생성 시 일괄 초기화합니다.

자원 데이터나 메시가 없고 후보 탐색 또는 생성이 실패해도 후속 수정된 자원 반복문은 목표 개수의 3배 시도 후 반환합니다. 목표 개수 미달은 로그로 남기며 부족한 개수를 반드시 채우지는 않습니다. 보급도 같은 자원 비트를 사용하며, 라운드 경계에서 보급 Actor를 제거하는 순서와 연결됩니다.
